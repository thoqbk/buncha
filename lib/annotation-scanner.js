...
function AnnotationScanner(logger, scannedAnnotations, $registerByConstructor, $unregister, $resolve) {
  ...
  */
  ...
  this.getServiceNamesByAnnotation = function (annotation) {
    return _(serviceNameNAnnotationMap)
    .keys()
    .filter(function (serviceName) {
      return serviceNameNAnnotationMap[serviceName] == annotation;
    });
  };
      ...
      var promise = new Promise(function (resolve, reject) {
        ...
        fs.stat(filePath, function (error, stats) {
          ...
          if (stats.isDirectory()) {
            ...
            var filePath2 = path.join(rootPath, filePath);
            fs.readFile(filePath2, "utf8", function (error, data) {
              if (error != null) {
                reject(error);
              } else {
